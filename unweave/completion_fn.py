...
class UnweaveBaseCompletionResult(CompletionResult):
...
class UnweaveChatCompletionResult(UnweaveBaseCompletionResult):
    def get_completions(self) -> list[str]:
        completions = []
...
class UnweaveCompletionResult(UnweaveBaseCompletionResult):
    def get_completions(self) -> list[str]:
        completions = []
        if self.raw_data and "choices" in self.raw_data:
            for choice in self.raw_data["choices"]:
...
class UnweaveCompletionFn(CompletionFn):
    def __init__(
        self,
    ...
    def __call__(
        ...
        **kwargs,
        ...
        # Dict imitating OpenAI result
        result = {"choices": [{"text": "stubbed response"}]}

        result = UnweaveCompletionResult(raw_data=result, prompt=openai_create_prompt)
        record_sampling(prompt=result.prompt, sampled=result.get_completions())
        return result


class UnweaveChatCompletionFn(CompletionFnSpec):
    def __init__(
        self,
        model: Optional[str] = None,
        api_base: Optional[str] = None,
    ...
    def __call__(
        ...
        **kwargs,
        ...
        # Dict imitating OpenAI result
        result = {"choices": [{"message": {"content": "stubbed response"}}]}

        result = UnweaveChatCompletionResult(raw_data=result, prompt=openai_create_prompt)
        record_sampling(prompt=result.prompt, sampled=result.get_completions())
        return result
